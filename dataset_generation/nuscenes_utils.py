...
class Scene:
    ...
    def add_CAN_data(self):
        ...
        for k,d in enumerate(self.data):
            ...
            pos = pose_data[i]['pos'][:2]
            yaw = quaternion_yaw(Quaternion(pose_data[i]['orientation']))
            pose = (pos[0], pos[1], yaw)
            self.data[k].update({'pose':pose})
    ...
    def add_ann_data(self):
        for d in self.data:
            ...
            for ann_token in sample['anns']:
                ...
                d['anns'].append({'token':ann_token, 'category':category, 'pose':(x, y, yaw)})
    ...
    def output_data(self):
        ...
        for i,d in enumerate(self.data):
            out.append({ 'map':self.map, 'scene':self.scene_token, 'pose':tuple(d['pose'][:3]), 'anns':d['anns'], 'action':flat_actions[i] })
        return out
